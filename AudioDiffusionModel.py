...
if __name__ == '__main__':
    ...
    # Old code
            ...
            #mel_spectrogram, phase = convert_waveform_to_mel_spectrogram(waveform, self.sample_rate, self.n_mels, self.fmax)

            # # # Compute the mel-spectrogram
            # # sgram = librosa.stft(waveform.numpy().squeeze())
            ...
            # # # Assuming mel_spectrogram is your input mel-spectrogram
            # # #mel_spectrogram = mel_spectrogram.numpy()

            # # # Convert mel-spectrogram from dB back to amplitude
            # # mel_spectrogram_amp = librosa.db_to_amplitude(mel_spectrogram)

            # # # Invert the mel-spectrogram to STFT
            # # sgram_mag = librosa.feature.inverse.mel_to_stft(mel_spectrogram_amp, sr=sample_rate, n_fft=2048, power=2.0)

            # # # Invert the STFT magnitude to waveform
            # # waveform_reconstructed = librosa.griffinlim(sgram_mag, n_iter=32, hop_length=128, win_length=2048)

            # # # Play the reconstructed audio
            # # play_audio_from_waveform(waveform_reconstructed, sample_rate)

            # # play_audio_from_waveform(waveform, sample_rate)
            ...
            # #     y=waveform.numpy().squeeze(),
            # #     sr=sample_rate,
            # #     n_fft=4096,          # Increase for better frequency resolution
            # #     hop_length=128,      # Decrease for better time resolution
            # #     win_length=4096,     # Same as n_fft to reduce phase artifacts
            # #     n_mels=self.n_mels,
            # #     fmax=self.fmax
            # # )
        ...
        # # def convert_to_mel_spectrogram(self, waveform, sample_rate, n_mels, fmax):
        # #     y = waveform.numpy().squeeze()
        # #     S = librosa.feature.melspectrogram(y=y, sr=sample_rate, n_mels=n_mels, fmax=fmax)
        # #     S_dB = librosa.power_to_db(S, ref=np.max)
        # #     return S_dB #torch.tensor(S_dB)
    ...
    # # # Process mel-spectrograms from the batch back into audio
    # # for i, mel_spectrogram in enumerate(batch['mel_spectrogram']):
    # #     # Convert mel-spectrogram from dB back to power
    # #     mel_spectrogram_power = librosa.db_to_power(mel_spectrogram.numpy())
        
    # #     # Reconstruct audio using mel-spectrogram
    # #     audio_signal = librosa.feature.inverse.mel_to_audio(
    # #         mel_spectrogram_power, 
    # #         sr=batch['sample_rate'][i].item(),
    # #         n_fft=4096,
    # #         hop_length=128,
    # #         win_length=4096,
    # #         n_iter=64 # Number of iterations for Griffin-Lim algorithm
    # #     )
    ...
    # #     # Play the reconstructed audio
    # #     play_audio_from_waveform(audio_signal, batch['sample_rate'][i].item())
    ...
    # Process mel-spectograms from the batch back into audio
    #for i, mel_spectrogram in enumerate(batch['mel_spectrogram']):#waveform in enumerate(batch['waveform']):
        #reconstructed_audio = mel_spectrogram_to_audio(mel_spectrogram.numpy(), batch['phase'][i].numpy(), batch['sample_rate'][i].item())
        #play_audio_from_waveform(reconstructed_audio, batch['sample_rate'][i].item())

        #play_audio_from_waveform(waveform.numpy().squeeze(), batch['sample_rate'][i].item())

        #librosa.output.write_wav(f'reconstructed_audio_{i}.wav', reconstructed_audio, batch['sample_rate'][i].item())


        # Process mel-spectrograms from the batch back into audio
    # # for i, mel_spectrogram in enumerate(batch['mel_spectrogram']):

    # #     # Convert mel-spectrogram to NumPy array with appropriate dtype
    # #     mel_spectrogram_np = mel_spectrogram.numpy().astype(np.float32)
        
    # #     reconstruction = reconstruct_waveform_from_mel_spectogram(mel_spectrogram_np, int(batch['sample_rate'][i].item())) 
        
    # #     # Play the reconstructed audio
    # #     play_audio_from_waveform(reconstruction, batch['sample_rate'][i].item())
